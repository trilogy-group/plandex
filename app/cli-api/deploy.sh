...
check_prerequisites() {
    if [ ! -f "$BINARY_PATH" ]; then
        ...
        print_info "Run 'go build -o plandex-cli-api' first"
        exit 1
    fi

    if [ ! -f "$CONFIG_PATH" ]; then
        print_error "Configuration not found at $CONFIG_PATH"
        print_info "Create configuration file first"
        exit 1
    fi
}
...
show_menu() {
    ...
    echo
    echo "Commands:"
    echo "  local     - Run API locally in foreground (Option 1)"
    echo "  autostart - Setup auto-start on boot with systemd (Option 3)"
